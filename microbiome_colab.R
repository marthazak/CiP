...
#import count table
asv <- read.table(url("https://raw.githubusercontent.com/marthazak/CiP/main/CiP_count_filtered.tsv"), header = TRUE, sep = "\t", row.names = 1)
head(asv)
asv.phylo	<-	otu_table(as.matrix(asv),	taxa_are_rows	=	TRUE)

#import taxonomy
tax <-	read.table(url("https://raw.githubusercontent.com/marthazak/CiP/main/CiP_taxonomy_filtered.tsv"),header=T , sep="\t", fill=TRUE, row.names=1)
tax.phylo <- tax_table(as.matrix(tax))
head(tax)

meta <- read.csv(url("https://raw.githubusercontent.com/marthazak/CiP/main/metadata.csv"),row.names=1)
head(meta)
